...
upload a file "foobar.txt" on a local computer to a remote host "hostname.org" using the username "user" to the /var/www directory
...
use SSH key based authentication then when you scp, scp will not ask for user's password. It will copy your file to the remote server using your ssh private key.
...
It is a good way to probe the existence of a firewall and its rule sets. If it finds a live host and an open port, it returns an RST response
...
At the first prompt hit enter (as the password is blank), at the second prompt you will set the password for your resulting encrypted disk.
