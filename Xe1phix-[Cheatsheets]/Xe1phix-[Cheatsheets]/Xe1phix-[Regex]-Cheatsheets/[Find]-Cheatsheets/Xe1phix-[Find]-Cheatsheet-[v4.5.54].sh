...
Print the list of all files owned by the user $User:
find . -type f -user $User -print


find all the files owned by root by using the -user option
and using -exec to perform the ownership change operation.
find . -type f -user root -exec chown $User {} \;
find . -type f -user root -exec chmod -v $User {} \;
...
Remove all the .$Ext files from the current directory as follows:
find . -type f -name "*.$Ext" -delete


find out the PHP files that dont have proper execute permissions as follows:
