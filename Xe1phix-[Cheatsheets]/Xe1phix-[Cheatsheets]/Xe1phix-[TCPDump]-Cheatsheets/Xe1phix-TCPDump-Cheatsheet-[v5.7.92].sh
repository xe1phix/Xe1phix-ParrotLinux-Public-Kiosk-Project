...
##-====================================================-##
## ---------------------------------------------------- ##
##   [?] Covers the IP header and typical TCP header.
##       (a.k.a.“snaplen of 56”.)
## ---------------------------------------------------- ##
...
##-==============================================-##
##   [+] Attempt to Capture Entire Contents of
##       Each packet (a.k.a “snaplen zero”.)
##-==============================================-##
...
dump  the  actual  traffic  captured  in  the  dump  file,  in  hex  format
...
## -------------------------------------- ##
##  network capture with filename based on tcpdump arguments
## -------------------------------------- ##
tcpdump -w "$(sed 's/-//gi; s/ /_/gi'<<<"-vvv -s0 -ieth1 -c10 icmp").pcap"


## -------------------------------------- ##
## print the hostname (--s short)
## and timestamp in the filename
## -------------------------------------- ##
tcpdump -i swp27 tcp port 179 -C 10 -w $(hostname -s).swp27-$(date +%Y-%m-%dT%H-%M-%S-%s)


## -------------------------------------- ##
##  [+] tcpdump capture filter: ICMPv6
## -------------------------------------- ##


##  [?] show only ICMPv6 echo-request and echo-reply packets\
tcpdump -i swp1 -ennvvv "(icmp6 && ip6[40] == 128) or (icmp6 && ip6[40] == 129)"

## show only ICMPv6 NS/NA packets\
tcpdump -i swp1 -ennvvv "(icmp6 && ip6[40] == 135) or (icmp6 && ip6[40] == 136)"
