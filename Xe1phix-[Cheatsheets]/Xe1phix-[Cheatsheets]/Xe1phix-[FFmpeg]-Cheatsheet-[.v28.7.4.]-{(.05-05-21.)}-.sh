...
ffmpeg -i $File.mp4 -acodec libmp3lame -b:a 256k -vn $File.mp3
...
ffmpeg -i $File.mp4 -acodec libmp3lame -b:a 320k -vn $File.mp3
...
echo "## -------------------------------------------------------------------------------------- ##"
ffmpeg -i $File.mp4 -keyint_min 150 -g 150 -f webm -vf setsar=1:1 $File.webm
...
echo "    [+] MP4 File --> H.264           "
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File.mp4 -c:v libx264 -c:a copy $File.mp4
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File.webm -v:c libvpx-vp9 -v:a copy $File.webm
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File.webm -v:c copy -v:a libvorbis $File.webm
ffmpeg -i $File.webm -v:c copy -v:a libopus $File.webm
...
encode as a lower quality+resolution

ffmpeg -i $File.mp4 -c:v libvpx -c:a libvorbis -b:v 1M -crf 30 -sn -vf scale=480:360 $File.webm
ffmpeg -i $File.mp4 -c:v libvpx -c:a libvorbis -b:v 1M -crf 30 -sn -vf scale=480:360 $File.webm




ffmpeg -i $File.webm -c:v h264 -b:v 1M -crf 30 -sn -vf scale=720:480 $File.mp4
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $input.mp3 -c:a libvorbis $File.ogg
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -loglevel repeat+level+verbose -i $File $File
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File -f ffmetadata $metadata.txt
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File -f ffmetadata FFMETADATAFILE
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -i $File -i FFMETADATAFILE -map_metadata 1 -codec copy $File
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -ss START -i "$File.mkv" $output.webm
ffmpeg -i "$File.mkv" -ss START $output.webm
...
echo "## ----------------------------------------------- ##"
ffmpeg -i $input.mov -c:v libx264 -preset slow -crf 18 -c:a aac -b:a 192k -pix_fmt yuv420p $File.mkv
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] Copy metadata from the first stream of the 
##      Input file To global metadata of the output file:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -i $File.ogg -map_metadata 0:s:0 $File.mp3


##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] Copy global metadata to all audio streams:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -i $File.mkv -map_metadata:s:a 0:g $File.mkv





ffmpeg -dump                ## Dump each input packet to stderr.

ffmpeg -hex                 ## When dumping packets, also dump the payload. (global)
...
echo "##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##"
ffmpeg -dump_attachment:t:0 $File.ttf -i $File
...
ffmpeg -i $File -map 0 -flags:v +global_header -c:v libx264 -bsf:v dump_extra $out.ts

ffmpeg -i $File.mp4 -codec copy -bsf:v h264_mp4toannexb $File.ts


ffmpeg -i $File -c:v copy -bsf:v 'filter_units=remove_types=35|38-40' $File



ffmpeg -i $File -c copy -bsf noise[=1] $File.mkv
...
ffmpeg -i $File -f ffmetadata $FFMETADATAFILE
ffmpeg -i $File -i $FFMETADATAFILE -map_metadata 1 -codec copy $OUTPUT
...
## ----------------------------------------------------- ##
##   [?] If you specify the input format and device 
##       then ffmpeg can grab video and audio directly.
## ----------------------------------------------------- ##
ffmpeg -f oss -i /dev/dsp -f video4linux2 -i /dev/video0 $File.mpg


## --------------------------------------- ##
##   [?] Or with an ALSA audio source 
##       (mono input, card id 1) 
##           instead of OSS:
## --------------------------------------- ##
ffmpeg -f alsa -ac 1 -i hw:1 -f video4linux2 -i /dev/video0 $File.mpg


ffmpeg -i $File.avi -codec copy -bsf:v mpeg4_unpack_bframes $File.avi
...
ffprobe -show_packets -select_streams v:1 $File
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
## Extract a representable text file from MOV subtitles


stripping the metadata header from each subtitle packet.
...
ffmpeg -i $File.avi -c:v copy -bsf:v mjpeg2jpeg frame_%d.jpg
exiftran -i -9 frame*.jpg
ffmpeg -i frame_%d.jpg -c:v copy $File.avi
...
##      (e.g. MPEG-2 sequence headers, or H.264/HEVC (VPS/)SPS/PPS) 
## ------------------------------------------------------------------- ##
##      to be transmitted either "in-band" 
##      (as a part of the bitstream containing the coded frames) 
## ------------------------------------------------------------------- ##
##      or "out of band" (on the container level). 
##      This latter form is called "extradata" in FFmpeg
## ------------------------------------------------------------------- ##
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] Extract the core from a DCA/DTS stream
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
## ----------------------------------------------- ##
##   [?] Dropping extensions such as DTS-HD.
## ----------------------------------------------- ##
dca_core
...
## ------------------------------------------------- ##
## a        [+] Add extradata to all key packets, 
##              but only if local_header is set 
...
## ------------------------------------------------------------------- ##
##   [?] Forces a global header 
## ------------------------------------------------------------------- ##
##   [?] Disabling individual packet headers in the H.264 packets
## ------------------------------------------------------------------- ##
##   [?] Generated by the "libx264" encoder
## ------------------------------------------------------------------- ##
##   [?] But its corrected by adding the header 
...
ffmpeg -i $video.webm -vcodec copy -an $video.webm
ffmpeg -i $video.webm -acodec copy -vn $audio.webm
...
##-============================================-##
##   [+] Change characteristics --> Bitrate
##-============================================-##

## -------------------------------------------------------- ##
##   [?] You can do this when converting to mp4 or webm.
## -------------------------------------------------------- ##
ffmpeg -i $video.mov -b:v 350K $video.mp4
ffmpeg -i $video.mov -vf setsar=1:1 -b:v 350K $video.webm
...
ffmpeg -i $video.mov -vf setsar=1:1 $video.webm
...
ffprobe -i $File.webm -show_streams
ffprobe -i $File.webm -show_streams | grep (width|height)

ffprobe -i jones.webm -show_streams | grep width
ffprobe -i jones.webm -show_streams | grep height
...
ffprobe -show_packets -select_streams v:1 -i jones.webm
...
##-=========================================-##
##   [+] Enables experimental HTTP server
##-=========================================-##


## --------------------------------------------------------------- ##
##   [?] This can be used to send data when used as an output
##       option, or read data from a client with HTTP POST 
##       when used as an input option.  If set to 2 enables
...
##-===============================================-##
##   [+] Define Your SSH Private Keys Location:
##-===============================================-##
private_key ~/.ssh/
...
cert=                   ## A file containing a certificate to use in the handshake with the peer.
                        ...
                        ## -------------------------------------------------------------- ##
key=                    ##   [?] A file containing the private key for the certificate
                        ## -------------------------------------------------------------- ##



listen=1                ## Server Role
listen=0                ## Client Role

## ---------------------------------------------------------------- ##
##   [?] If enabled, listen for connections on the provided port, 
##       Assume the server role, 
##       (in the handshake) instead of the client role.
## ---------------------------------------------------------------- ##
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] As the previous example, but use filename for specifying 
##      the graph description, and omit the "out0" label:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] Read an audio stream from a file using the amovie source 
##      and play it back with ffplay:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] Capture from two devices simultaneously, 
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -f x11grab -framerate 25 -video_size cif -i :0.0+10,20 $File.mpg

ffmpeg -f x11grab -follow_mouse centered -framerate 25 -video_size cif -i :0.0 $File.mpg
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -f x11grab -follow_mouse 100 -framerate 25 -video_size cif -i :0.0 $File.mpg
...
ffprobe -v error -select_streams v:0 -show_entries stream=width,height,duration,bit_rate -of default=noprint_wrappers=1 $File.mp4

ffprobe -v error -select_streams v:0 -show_format $File.webm

ffprobe -v error -select_streams v:0 -show_streams $File.webm
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##  [+] copy with ffmpeg the entire Audio-CD in /dev/sr0, you may run the command:
...
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##   [+] Record video from video device 0 
##   [+] and audio from audio device 0 into out.avi:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -f avfoundation -i "0:0" out.avi

##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##   [+] Record video from video device 2 
##   [+] and audio from audio device 1 into out.avi:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
ffmpeg -f avfoundation -video_device_index 2 -i ":1" out.avi

##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
##   [+] Record video from the system default video device 
##   [+] using the pixel format bgr0 
##   [+] Dont record any audio into out.avi:
##-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-##
...
for f in *.mkv; do ffmpeg -y -i \"$f\" -map 0:m:language:eng -map -0:a -map -0:v \"${f%.*}.en.srt\"; done;





convert mkv to apple tv compatible mp4
ffmpeg -i INPUT.mkv -map 0:v:0  -c:v copy -map 0:a:0  -c:a libfdk_aac -b:a 160k  -ac 2 -vtag hvc1 -y output.mp4



convert x265 to mp4 for quicktime compatability.  use dolby eac3 for multistream audio support"
ffmpeg -i INPUT.mkv -map 0:v:0  -c:v copy -map 0:a:0  -c:a eac3 -vtag hvc1 -y output.mp4


## ffmpeg: cut 15 seconds from the start and 30 seconds from the end for every mkv video file in a folder.  (ffmpeg doesnt have an simple \N seconds from end\ param)
for f in ./*.mkv; do ffmpeg -i \"$f\" -ss 00:00:15 -to  $(( $(ffprobe -v 0 -show_entries format=duration -of compact=p=0:nk=1 $f) - 30 )) -c copy  \"parts/${f%.*}-part.mkv\"; done



##  ffmpeg: concat all `*part*.mkv` video files into one big video file. 
##  Very fast. No transcoding, but videos must have compataible encodings to begin with.  
##  Subtitles get combined just fine.  ffmpegs conact takes the list of files in a weird way so thats generated on the fly.
ffmpeg -safe 0 -f concat -i  <(for f in *part*.mkv;  do echo file \"$(pwd)/$f\"; done) -c copy ./all-video-parts-combined.mkv


##  side by side video and spectrum of its audio https://video.stackexchange.com/questions/23222/ffplay-is-there-a-way-to-display-a-video-and-spectrogram-in-the-same-output
ffplay -f lavfi 'amovie=sweet-video.mkv:s=dv+da[v][a];[a]asplit[asp][out1];[asp]showspectrum=size=660x427:mode=separate:slide=scroll[spect]; [v]scale=640:-1[v];[v][spect]hstack[out0]'


##  record webcam and microphone with ffmpeg.  Real world timestamp overlaid. Tuned for very low cpu usage and moderate file size/quality good for making a day-line recording of yourself.  (12 hours = 10GB).  hevc_videotoolbox is the key to making it very low cpu.  If you drop the microphone recording the CPU goes down by 50%... it's is a bit dumb though, so the bitrate is still rather high.  0.6M instead of 1.2MBits would make it roughly half as big and okay quality (except when there's motion/light changes).
ffmpeg   -f avfoundation  -framerate 15 -s \"800x600\" -i \"0:3\"  -vf \"settb=AVTB,setpts='trunc(PTS/1K)*1K+st(1,trunc(RTCTIME/1K))-1K*trunc(ld(1)/1K)',drawtext=text='%{localtime}.%{eif\\:1M*t-1K*trunc(t*1K)\\:d}'\"  -c:v hevc_videotoolbox  -realtime 1 -c:a libopus -compression_level 0 -application lowdelay -ac 1 -b:a 32k -b:v 1.2M -tune zerolatency -preset ultrafast  -threads 1 -y /tmp/webcam.mkv

##  record screen capture, webcam, and loopback audio into one file.  Needs lots of tweaking.  Probably use the hardware encoders
ffmpeg -f avfoundation -i \"2:3\" -f avfoundation -framerate 24 -s \"1280x720\" -i \"0\" -filter_complex \"[0:v]scale=1280:-1[scd];[scd][1:v]vstack\"   ~/Downloads/media/live2.mkv

##  ffmpeg/ffprobe: print timestamp of every frame from first 5 mins, then us awk to filter for key frames only (K)
ffprobe -loglevel error -select_streams v:0 -read_intervals '%05:00' -show_entries packet=pts_time,flags -of csv=print_section=0 input.mkv | awk -F',' '/K/ {print $1}'
...
##  output a report to a file named ffreport.log 
