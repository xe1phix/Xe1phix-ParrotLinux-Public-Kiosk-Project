...
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "smtp" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." 

msfcli $a LHOST=192.168.1.145 RHOST=$TARGET RHOSTS=$TARGET RPORT=25 USER_FILE=$USER_FILE PASS_FILE=$PASS_FILE THREADS=$THREADS E; done;
...
samrdump.py $TARGET
nbtscan $TARGET
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "smb" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." && 

msfcli $a LHOST=192.168.1.145 RHOST=$TARGET RHOSTS=$TARGET RPORT=139 USER_FILE=$USER_FILE PASS_FILE=$PASS_FILE THREADS=$THREADS VERBOSE=false E; done;
...
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "nfs" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." 
msfcli $a LHOST=192.168.1.145 RHOST=$TARGET RHOSTS=$TARGET RPORT=111 USER_FILE=$USER_FILE PASS_FILE=$PASS_FILE THREADS=$THREADS E; done;
...
rpcinfo -p $TARGET
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "rpc" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." 

msfcli $a LHOST=192.168.1.145 RHOST=$TARGET RHOSTS=$TARGET RPORT=135 USER_FILE=$USER_FILE PASS_FILE=$PASS_FILE THREADS=$THREADS E; done;
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "dce" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." 
...
echo -e "Starting XSSer..." 

xsser -u http://$TARGET:8000 -c10 --Cw=200 --auto --save --follow-redirects | egrep "Injection:|Final Results:|Injections:|Failed:|Successfull:|Accur:"
...
for a in `cat $FINDSPLOIT_DIR/msf_search/auxiliary | egrep "pop" | egrep "scanner" | awk '{print $1}'`; do echo -e "$COLOR2Running Metasploit module: $a..." 

msfcli $a LHOST=192.168.1.145 RHOST=$TARGET RHOSTS=$TARGET RPORT=110 USER_FILE=$USER_FILE PASS_FILE=$PASS_FILE THREADS=$THREADS E; done;
...
compare our Nmap scan results for port 80 with a 
Metasploit scanning module
determine what hosts had port 80 open
according to Nmap.
...
enum_con gs module collects con guration les found on commonly installed applications and services, such as Apache, MySQL, Samba, Sendmail, etc
...
##  Pentest_Compilation-[massive_escopeting].sh
https://raw.githubusercontent.com/adon90/pentest_compilation/master/Recon/massive_escopeting.sh
...
I used tcpdump over ssh to pass all the traffic as pcap data 
to a Linux system that then takes the pcap data 
and passes it to snort for processing.
...
##  Start Suricata:
suricata -c /etc/suricata.yaml -i eth0 -D
...
tcpdump -tnn -c 100000 -i eth2 | awk -F "." '{print $1"."$2"."$3"."$4}' | sort | uniq -c | sort -nr | awk ' $1 > 100 '
...
## ------------------------------------------------------------------------- ##
##    [+] Move Certificate Into The Hexchat Directory:
## ------------------------------------------------------------------------- ##
...
https://www.oftc.net/NickServ/CertFP/#thunderbird
...
##-===================================================-##
##      Xe1phix-Mullvad+OpenVPN-ProductionNotes.sh
##-===================================================-##
...
## ========================================= ##
## I2P Subscriptions Txt Configurations:
## 
## Xe1phix-I2P-Router-AddressBook+Inr.txt
## 
...
## 			http://inr.i2p/browse/alive-hosts.txt
## ================================== ##
...
## ------------------------------------------------------------------------------------------------------------------ ##
## http://inr.i2p/browse/				##  I2P Name Registry - A domain name registration service for I2P
## ------------------------------------------------------------------------------------------------------------------ ##
...
# Verify there is a cron job scheduled to run the aide check
  crontab -u root -l | cut -d\# -f1 | grep -q "aide \+--check"
...
https://github.com/topics/cis-benchmark?l=shell&o=desc&s=forks

https://github.com/topics/hardening?l=shell
https://github.com/topics/hardening?l=shell&o=desc&s=forks
https://github.com/topics/hardening?l=shell&o=desc&s=forks
...
https://github.com/search?l=Shell&p=2&q=audit&type=Repositories



https://github.com/mksipe/Shell-Script-Lib
...
Awesome-One-liner-Bug-Bounty.md

https://github.com/dwisiswant0/awesome-oneliner-bugbounty
https://raw.githubusercontent.com/dwisiswant0/awesome-oneliner-bugbounty/master/README.md



https://github.com/finalduty/cis_benchmarks_audit
https://github.com/finalduty/cis_benchmarks_audit/blob/trunk/cis-audit.sh
[cis_benchmarks_audit]-cis-audit.sh





https://github.com/XalfiE/Bug-Bounty-Oneliners/blob/master/OneLiners
