...
youtube-dl --verbose --continue --no-overwrites --ignore-errors --force-ipv4 --ffmpeg-location /usr/bin/ffmpeg --newline --print-traffic -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best' -o '%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s' 
...
youtube-dl --verbose --force-ipv4 --continue --no-overwrites --ignore-errors --extract-audio --audio-quality 3 --audio-format mp3 --ffmpeg-location /usr/bin/ffmpeg --newline -o '%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s' 
